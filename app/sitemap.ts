...
export default function sitemap(): MetadataRoute.Sitemap {
  ...
  return [...staticRoutes, ...conceptRoutes, ...patientGuideRoutes, ...casebankCases.map((item) => "/casebank/" + item.slug), "/videos", ...operativeVideos.map((video) => "/videos/" + video.slug)].map((route) => ({
    url: absoluteUrl(route),
    lastModified: new Date(route.startsWith("/videos") || route === "/casebank/right-l5s1-far-lateral-discectomy" || route === "/operative-concepts/ube-far-lateral-l5s1" ? videoPageUpdated : route === "/casebank" ? casebankArchive.updated : route.startsWith("/casebank/") ? casebankUpdated : route.startsWith("/patient-education") ? patientEducationReviewDate : route.startsWith("/operative-concepts/") ? pageDates["/operative-concepts/[slug]"] : (pageDates as Record<string, string>)[route] ?? "2026-08-24"),
