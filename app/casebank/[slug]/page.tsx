...
export default async function CasePage({ params }: Props) {
  ...
  return <>
    ...
    <PageHeader compact eyebrow={record.caseNumber + " · " + record.region + " · " + record.procedureGroup} title={record.shortTitle} description={record.question}>
      <p className="mt-4 text-sm text-slate-600">Source: Hanjin Jang, MD academic case collection · Page updated {record.updated}</p>
    </PageHeader>
    <article className="mx-auto max-w-5xl px-5 py-12">
      ...
      <nav aria-label="On this case page" className="my-7 flex flex-wrap gap-x-6 gap-y-3 text-sm font-semibold text-academic-navy"><a href="#clinical-reasoning">Clinical reasoning</a><a href="#images">Clinical images</a><a href="#outcomes">Reported observations</a><a href="#evidence">Evidence</a><a href="#provenance">Source and limitations</a></nav>
      ...
      <section id="images" className="mt-10 border-t border-academic-line pt-8">
        <h2 className="font-serif text-3xl text-academic-navy">Published clinical images</h2>
        <p className="mt-3 text-sm leading-7 text-slate-600">Images are retained from the existing public educational collection. Read each caption and use the original view to inspect the image. Imaging changes alone do not measure pain relief, function, or durable fusion.</p>
        ...
        {!record.images?.length ? <p className="mt-5">{notReported}</p> : null}
      </section>
      <section id="outcomes" className="mt-10 border-t border-academic-line pt-8">
        ...
        <p className="mt-3 text-sm leading-7 text-slate-600">Values below are transcribed from the existing public case summary; source charts were not re-audited for this website update. Operative-time definitions and observation windows may differ. Do not compare these values across surgeons or procedures.</p>
      ...
      <section id="provenance" className="mt-10 border border-academic-line bg-academic-panel p-6">
        ...
        <p className="mt-3 text-sm leading-7 text-slate-700">Source material: {record.caseNumber} in the previously published Case-Based Education collection by Hanjin Jang, MD. This record was reorganized on {record.updated}; this date records a website update, not a new clinical review. A separate case-level clinical review date and public consent record are not reported in the available source.</p>
