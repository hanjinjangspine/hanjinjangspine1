...
export default function VideoArchivePage() {
  return <>
    ...
    <section className="mx-auto max-w-6xl px-5 pb-12">
      <div className="mb-8 border-l-2 border-academic-gold bg-academic-panel p-5 text-sm leading-7 text-slate-600"><p><strong>{archiveVideos.length} public excerpts from a {casebankArchive.videoCatalogueCases}-record catalogue.</strong> One public entry represents one catalogue record; alternate files are not counted again. This is a video archive, and a record is not proof of a unique patient, a complete operation or a clinical outcome. Its overlap with the <Link href="/casebank#published-cases" className="underline">nine detailed teaching entries</Link> is not established.</p><p className="mt-3">Publication: {videoArchiveUpdated}. The collection owner authorized publication of eligible material. {casebankArchive.videoExcludedRecords} catalogue records are excluded: three without an available nonempty original, three with unusable video data, two shorter than two seconds, and one without an identifiable operative scene. Published excerpts omit audio and source identifiers. Recorded procedure labels and levels are retained as source labels; an independent clinical review and patient-level outcome record are not available for these entries.</p></div>
