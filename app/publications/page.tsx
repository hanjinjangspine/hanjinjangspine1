...
export default function PublicationsPage() {
  return <>
    <JsonLd data={{ "@context": "https://schema.org", "@type": "CollectionPage", name: "Selected Publications — Hanjin Jang, MD", url: absoluteUrl("/publications"), dateModified: evidenceCheckedDate, mainEntity: { "@type": "ItemList", numberOfItems: publications.length, itemListElement: publications.map((item, index) => ({ "@type": "ListItem", position: index + 1, item: { "@type": "ScholarlyArticle", name: item.title, url: item.href, identifier: item.doi } })) } }} />
    ...
    <section className="mx-auto max-w-5xl px-5 py-12">
      ...
      <div className="grid gap-7">{publications.map((item) => <article key={item.id} className="border border-academic-line p-6 sm:p-8">
