    ...
    <div className="mx-auto max-w-6xl px-5 py-12">
      <div className="grid gap-8 lg:grid-cols-[1.3fr_1fr]">
        ...
        <dl className="grid grid-cols-3 items-center gap-4 border-l-2 border-academic-gold pl-6">
          {[ [casebankCards.length, "Educational cases"], [publications.length, "Selected publications"], [operativeConcepts.length, "Operative concepts"] ].map(([count,label]) => <div key={label}><dt className="text-xs leading-6 text-slate-600">{label}</dt><dd className="mt-2 font-serif text-4xl text-academic-navy">{count}</dd></div>)}
        </dl>
      </div>
      ...
      <p className="mt-5 text-xs leading-6 text-slate-500">Counts describe this website&apos;s educational collection, not surgical volume or treatment outcomes.</p>
    </div>
  </section>;
}
